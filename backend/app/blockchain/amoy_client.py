...
def submit_root_hash_to_amoy(root_hash: str) -> Dict[str, Any]:
    ...
    Uses multi-RPC fallback list for network resilience.
    """
    if settings.WALLET_PRIVATE_KEY:
        from web3 import Web3
        for rpc_url in AMOY_RPC_NODES:
            if not rpc_url:
                continue
            try:
                ...
                if w3.is_connected():
                    ...
                    return {
                        "network": f"Polygon Amoy Testnet ({rpc_url})",
                        "chain_id": 80002,
                        "root_hash": root_hash,
                        "tx_hash": tx_hex,
                ...
                continue  # Try next RPC node in list
